...
function initCardImageGenerator() {
    ...
    function draw() {
        ...
        // REPLACE your current italicSubstrings + writeLineWithIconsReplacedWithSpaces(...) block with this:

var shadowDistance = 10;
var italicSubstrings = ["[i]", "Heirloom: ", "Erbstück: ", "(This is not in the Supply.)", "Keep this until Clean-up."];
var boldSubstrings = ["[b]"]; // NEW: lines containing [b] render bold
...
function writeLineWithIconsReplacedWithSpaces(line, x, y, scale, family, boldSize) {
    ...
    if (italicSubstrings.some(substring => line.includes(substring))) {
        if (!context.font.startsWith("italic ")) context.font = "italic " + context.font;

        if (line.includes("[i]")) {
            line = line.split("[i]").join("");
            x += boldSize * scale; // keep your original indent behavior
        }
    } else {
        if (context.font.startsWith("italic ")) context.font = context.font.replace(/^italic\s+/, "");
    }

    // ---- bold handling ([b]) ----
    if (boldSubstrings.some(substring => line.includes(substring))) {
        if (!context.font.startsWith("bold ")) context.font = "bold " + context.font;

        if (line.includes("[b]")) {
            line = line.split("[b]").join("");
            x += boldSize * scale; // matches [i] indent; delete this line if you don't want indent
        }
    } else {
        if (context.font.startsWith("bold ")) context.font = context.font.replace(/^bold\s+/, "");
    }
